...
namespace
{
    ...
    struct count_layout
    {
        abt_boost::detail::sp_counted_base * pi;
        int id;
    };
...
static void scan_and_mark(void const * area, size_t size, map2_type & m2, open_type & open)
{
    ...
    for(size_t n = 0; n + sizeof(shared_ptr_layout) <= size; p += pointer_align, n += pointer_align)
    {
        ...
        if(q->pn.id == abt_boost::detail::shared_count_id && q->pn.pi != 0 && m2.count(q->pn.pi) != 0)
        {
...
typedef std::deque< abt_boost::shared_ptr<X> > free_list_type;

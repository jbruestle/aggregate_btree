...
#define BOOST_BGL_ONE_PARAM_REF(name, key) \
    ...
    bgl_named_params<abt_boost::reference_wrapper<PType>, BOOST_PP_CAT(key, _t), self> \
    name(PType& p) const { \
      typedef bgl_named_params<abt_boost::reference_wrapper<PType>, BOOST_PP_CAT(key, _t), self> Params; \
      return Params(abt_boost::ref(p), *this); \
    } \
...
#define BOOST_BGL_ONE_PARAM_REF(name, key) \
    template <typename PType> \
    bgl_named_params<abt_boost::reference_wrapper<PType>, BOOST_PP_CAT(key, _t)> \
    name(PType& p) { \
      typedef bgl_named_params<abt_boost::reference_wrapper<PType>, BOOST_PP_CAT(key, _t)> Params; \
      return Params(abt_boost::ref(p)); \
    } \
...
#define BOOST_BGL_ONE_PARAM_REF(name, key) \
    ...
    struct convert_one_keyword<BOOST_PP_CAT(key, _t)> { \
      typedef abt_boost::graph::keywords::tag::name type; \
    };
...
#undef BOOST_BGL_ONE_PARAM_CREF
    ...
    struct convert_bgl_params_to_boost_parameter {
      ...
      typedef abt_boost::parameter::aux::tagged_argument<new_kw, const typename T::value_type> tagged_arg_type;
      typedef convert_bgl_params_to_boost_parameter<typename T::next_type> rest_conv;
      typedef abt_boost::parameter::aux::arg_list<tagged_arg_type, typename rest_conv::type> type;
    ...
    template <>
    struct convert_bgl_params_to_boost_parameter<abt_boost::no_property> {
      typedef abt_boost::parameter::aux::empty_arg_list type;
      static type conv(const abt_boost::no_property&) {return type();}
    };

    template <>
    struct convert_bgl_params_to_boost_parameter<abt_boost::no_named_parameters> {
      typedef abt_boost::parameter::aux::empty_arg_list type;
      static type conv(const abt_boost::no_property&) {return type();}
    };
    ...
    struct parameter_exists : abt_boost::mpl::not_<abt_boost::is_same<typename abt_boost::parameter::binding<ArgPack, KeywordType, bgl_parameter_not_found_type>::type, bgl_parameter_not_found_type> > {};
  }

#define BOOST_GRAPH_DECLARE_CONVERTED_PARAMETERS(old_type, old_var) \
  typedef typename abt_boost::detail::convert_bgl_params_to_boost_parameter<old_type>::type arg_pack_type; \
  arg_pack_type arg_pack = abt_boost::detail::convert_bgl_params_to_boost_parameter<old_type>::conv(old_var);

  namespace detail {

    template <typename ArgType, typename Prop, typename Graph, bool Exists>
    struct override_const_property_t {
      typedef typename abt_boost::remove_const<ArgType>::type result_type;
    ...
    struct override_const_property_t<ArgType, Prop, Graph, false> {
      typedef typename abt_boost::property_map<Graph, Prop>::const_type result_type;
    ...
    struct override_const_property_result {
      typedef 
        typename override_const_property_t<
                   typename abt_boost::parameter::value_type<ArgPack, Tag, int>::type,
                   Prop,
                   Graph,
                   abt_boost::detail::parameter_exists<ArgPack, Tag>::value
                 >::result_type
        type;
    };
    ...
    override_const_property(const ArgPack& ap, const abt_boost::parameter::keyword<Tag>& t, const Graph& g, Prop) {
    ...
    struct override_property_t {
      ...
      result_type operator()(const Graph& g, const typename abt_boost::add_reference<ArgType>::type a) const {return a;}
    };

    template <typename ArgType, typename Prop, typename Graph>
    struct override_property_t<ArgType, Prop, Graph, false> {
      typedef typename abt_boost::property_map<Graph, Prop>::type result_type;
    ...
    override_property(const ArgPack& ap, const abt_boost::parameter::keyword<Tag>& t, const Graph& g, Prop prop) {
  ...
  namespace detail {
    ...
    struct map_maker_helper<false, Graph, ArgPack, Value, PM> {
      typedef typename abt_boost::remove_const<
        typename override_const_property_t<
          typename abt_boost::parameter::value_type<
            ArgPack, abt_boost::graph::keywords::tag::vertex_index_map, int>::type,
          abt_boost::vertex_index_t,
          Graph,
          abt_boost::detail::parameter_exists<
            ArgPack, abt_boost::graph::keywords::tag::vertex_index_map>::value
        >::result_type>::type vi_map_type;
      typedef
        abt_boost::shared_array_property_map<Value, vi_map_type>
        map_type;
      static map_type make_map(const Graph& g,
        ...
        return make_shared_array_property_map(
                 ...
                 override_const_property(
                   ap,
                   abt_boost::graph::keywords::_vertex_index_map,
                   g, vertex_index));
      }
    };
    ...
    struct map_maker {
      ...
      typedef map_maker_helper<has_map, Graph, ArgPack, ValueType,
                               typename abt_boost::remove_const<
                                 typename abt_boost::parameter::value_type<
                                            ArgPack,
                                            MapTag,
                                            int
      ...
      static map_type make_map(const Graph& g, const ArgPack& ap, ValueType default_value) {
        return helper::make_map(g, default_value, ap[::abt_boost::parameter::keyword<MapTag>::instance | 0], ap);
      }
    };
    ...
    static const
      make_property_map_from_arg_pack_gen<
        abt_boost::graph::keywords::tag::color_map,
        default_color_type>
    ...
    struct priority_queue_maker_helper<false, Graph, ArgPack, KeyT, ValueT, KeyMapTag, IndexInHeapMapTag, Compare, Q> {
      ...
      typedef abt_boost::d_ary_heap_indirect<ValueT, 4, index_in_heap_map, typename map_maker<Graph, ArgPack, KeyMapTag, KeyT>::helper::map_type, Compare> priority_queue_type;

      static priority_queue_type
      make_queue(const Graph& g, const ArgPack& ap, KeyT defaultKey, const Q& q) {
        return priority_queue_type(
            map_maker<Graph, ArgPack, KeyMapTag, KeyT>::make_map(g, ap, defaultKey),
            map_maker<Graph, ArgPack, IndexInHeapMapTag, default_index_in_heap_type>::make_map(g, ap, typename abt_boost::property_traits<index_in_heap_map>::value_type(-1))
          );
      }
    };
    ...
    struct priority_queue_maker {
      BOOST_STATIC_CONSTANT(
        ...
        g_hasQ =
          (parameter_exists<ArgPack, PriorityQueueTag>
           ::value));
      typedef abt_boost::reference_wrapper<int> int_refw;
      typedef typename abt_boost::parameter::value_type<
                         ArgPack,
                         PriorityQueueTag,
                         int_refw
                       >::type
      ...
      typedef typename param_value_type_wrapper::type
        param_value_type;
      typedef typename abt_boost::remove_const<param_value_type>::type param_value_type_no_const;
      ...
      static priority_queue_type make_queue(const Graph& g, const ArgPack& ap, KeyT defaultKey) {
        return helper::make_queue(g, ap, defaultKey, ap[::abt_boost::parameter::keyword<PriorityQueueTag>::instance | 0]);
      }
    };

    template <class PriorityQueueTag, class KeyT, class ValueT, class Compare = std::less<KeyT>, class KeyMapTag = abt_boost::graph::keywords::tag::distance_map, class IndexInHeapMapTag = abt_boost::graph::keywords::tag::index_in_heap_map>

...
#define BOOST_TYPEOF_modifier_support(ID, Fun)\
    ...
    {\
        typedef\
            typename abt_boost::type_of::encode_type<\
            typename abt_boost::type_of::push_back<\
            V\
            , abt_boost::mpl::size_t<ID> >::type\
            , T>::type\
            type;\
    };\
    template<class Iter> struct decode_type_impl<abt_boost::mpl::size_t<ID>, Iter>\
    {\
        typedef abt_boost::type_of::decode_type<Iter> d1;\
...
#define BOOST_TYPEOF_array_support(ID, Qualifier)\
    ...
    {\
        typedef\
            ...
            V\
            , abt_boost::mpl::size_t<ID> >::type\
            , abt_boost::mpl::size_t<N> >::type\
            , T>::type\
        type;\
    };\
    template<class Iter>\
    struct decode_type_impl<abt_boost::mpl::size_t<ID>, Iter>\
    {\
        enum{n = Iter::type::value};\
        typedef abt_boost::type_of::decode_type<typename Iter::next> d;\

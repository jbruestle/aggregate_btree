...
#define BOOST_TYPEOF_TEMPLATE_PARAM_ENCODE(This, n)\
   typedef typename abt_boost::type_of::encode_template<BOOST_PP_CAT(V, n),\
...
#define BOOST_TYPEOF_TEMPLATE_PARAM_DECODE(This, n)\
   typedef abt_boost::type_of::decode_template< BOOST_PP_CAT(iter, n) > BOOST_PP_CAT(d, n);\
...
#define BOOST_TYPEOF_REGISTER_TYPE_FOR_TEMPLATE_TEMPLATE(Name,Params,ID)\
    ...
    struct encode_template_impl<V,Name<\
        ...
        P)> >\
        : abt_boost::type_of::push_back<V, abt_boost::mpl::size_t<ID> >\
    {\
    };\
    template<class Iter> struct decode_template_impl<abt_boost::mpl::size_t<ID>, Iter>\
    {\

...
using abt_boost::graph::parallel::trigger_receive_context;
 ...
 public:
  ...
  { 
    send(data->process_group, p, property_map_put, 
         abt_boost::parallel::detail::make_untracked_pair(k, v)); 
  }
 ...
 private:
  ...
  struct handle_message
  {
    ...
    handle_put(int source, int tag, 
               const abt_boost::parallel::detail::untracked_pair<key_type, value_type>& data, 
...
namespace abt_boost{} namespace boost = abt_boost; namespace abt_boost{
  ...
  struct hash<abt_boost::detail::edge_desc_impl<D, V> >
  {
    std::size_t operator()(const abt_boost::detail::edge_desc_impl<D, V> & x) const

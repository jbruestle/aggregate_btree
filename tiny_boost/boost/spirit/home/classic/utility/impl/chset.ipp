...
namespace utility { namespace impl {
    ...
    inline void
    detach(abt_boost::shared_ptr<basic_chset<CharT> >& ptr)
    {
        if (!ptr.unique())
            ptr = abt_boost::shared_ptr<basic_chset<CharT> >
                (new basic_chset<CharT>(*ptr));
    }

    template <typename CharT>
    inline void
    detach_clear(abt_boost::shared_ptr<basic_chset<CharT> >& ptr)
    {
        if (ptr.unique())
            ptr->clear();
        else
    ...
    void construct_chset(abt_boost::shared_ptr<basic_chset<CharT> >& ptr,
    ...
    void chset_negated_set(abt_boost::shared_ptr<basic_chset<CharT> > &ptr, chlit<CharT> const &ch,
            FakeT)
    {
    ...
    template <typename CharT, typename FakeT>
    void chset_negated_set(abt_boost::shared_ptr<basic_chset<CharT> > &ptr,

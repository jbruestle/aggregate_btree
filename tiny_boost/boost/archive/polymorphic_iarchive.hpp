...
public:
    ...
    virtual void load(abt_boost::long_long_type & t) = 0;
    virtual void load(abt_boost::ulong_long_type & t) = 0;
    ...
    void load_override(
        ...
        const
        #endif
        abt_boost::serialization::nvp< T > & t,
        int
    ){
        load_start(t.name());
...
public:
    ...
    virtual const detail::basic_pointer_iserializer * load_pointer(
        ...
        const detail::basic_pointer_iserializer * (*finder)(
            const abt_boost::serialization::extended_type_info & type
        )
    ) = 0;
};
...
// required by export
BOOST_SERIALIZATION_REGISTER_ARCHIVE(abt_boost::archive::polymorphic_iarchive)

...
public:
    ...
    {
      if (x.count())
        save_impl(x.address(), abt_boost::mpi::get_mpi_datatype(*x.address()), x.count());
    }
    ...
    MPI_Datatype get_mpi_datatype()
    {
      if (!is_committed)
      {
        BOOST_MPI_CHECK_RESULT(MPI_Type_struct,
                    (
                      addresses.size(),
                      abt_boost::serialization::detail::get_data(lengths),
                      abt_boost::serialization::detail::get_data(addresses),
                      abt_boost::serialization::detail::get_data(types),
                      &datatype_
                    ));
    ...
    void save(const T & t)
    {
        save_impl(&t, abt_boost::mpi::get_mpi_datatype(t), 1);
    }

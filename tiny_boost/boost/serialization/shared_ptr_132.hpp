...
inline void serialize(
    Archive & /* ar */,
    abt_boost_132::detail::sp_counted_base_impl<P, D> & /* t */,
    const unsigned int /*file_version*/
){
    // register the relationship between each derived class
    // its polymorphic base
    abt_boost::serialization::void_cast_register<
        abt_boost_132::detail::sp_counted_base_impl<P, D>,
        abt_boost_132::detail::sp_counted_base 
    >(
        static_cast<abt_boost_132::detail::sp_counted_base_impl<P, D> *>(NULL),
        static_cast<abt_boost_132::detail::sp_counted_base *>(NULL)
    );
}
...
inline void save_construct_data(
    ...
    ar << abt_boost::serialization::make_nvp("ptr", t->ptr);
}
...
inline void load_construct_data(
    ...
    P ptr_;
    ar >> abt_boost::serialization::make_nvp("ptr", ptr_);
    // ::new(t)abt_boost_132::detail::sp_counted_base_impl<P, D>(ptr_,  D()); 
    // placement
    ...
    // new system as well.
    ::new(t)abt_boost_132::detail::sp_counted_base_impl<
        P, 
        abt_boost_132::serialization::detail::null_deleter
    >(
        ptr_,  abt_boost_132::serialization::detail::null_deleter()
    ); // placement new
...
inline void save(
    Archive & ar,
    const abt_boost_132::detail::shared_count &t,
    const unsigned int /* file_version */
){
    ar << abt_boost::serialization::make_nvp("pi", t.pi_);
}

template<class Archive>
inline void load(
    Archive & ar,
    abt_boost_132::detail::shared_count &t,
    const unsigned int /* file_version */
){
    ar >> abt_boost::serialization::make_nvp("pi", t.pi_);
    if(NULL != t.pi_)
...
BOOST_SERIALIZATION_SPLIT_FREE(abt_boost_132::detail::shared_count)
...
inline void save(
    Archive & ar,
    const abt_boost_132::shared_ptr< T > &t,
    const unsigned int /* file_version */
){
    // only the raw pointer has to be saved
    // the ref count is maintained automatically as shared pointers are loaded
    ar.register_type(static_cast<
        abt_boost_132::detail::sp_counted_base_impl<T *, abt_boost::checked_deleter< T > > *
    >(NULL));
    ar << abt_boost::serialization::make_nvp("px", t.px);
    ar << abt_boost::serialization::make_nvp("pn", t.pn);
}

template<class Archive, class T>
inline void load(
    Archive & ar,
    abt_boost_132::shared_ptr< T > &t,
    const unsigned int /* file_version */
){
    // only the raw pointer has to be saved
    // the ref count is maintained automatically as shared pointers are loaded
...
inline void serialize(
    ...
    BOOST_STATIC_ASSERT(
        ...
        != abt_boost::serialization::track_never
    );
    abt_boost::serialization::split_free(ar, t, file_version);
}
...
#define BOOST_SHARED_POINTER_EXPORT_GUID(T, K)                     \
    typedef abt_boost_132::detail::sp_counted_base_impl<               \
        T *,                                                       \
        abt_boost::checked_deleter< T >                                \
    > __shared_ptr_ ## T;                                          \

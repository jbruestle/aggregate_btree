...
inline void save_collection(Archive & ar, const Container &s)
{
    ...
    #if 0
        abt_boost::archive::library_version_type library_version(
            ar.get_library_version()
        );
        if(abt_boost::archive::library_version_type(3) < library_version){
    ...
    while(count-- > 0){
        ...
        abt_boost::serialization::save_construct_data_adl(
            ar, 
            &(*it), 
            item_version
        );
        ar << abt_boost::serialization::make_nvp("item", *it++);
    }
}

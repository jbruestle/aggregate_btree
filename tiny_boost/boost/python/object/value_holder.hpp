...
{
    if (void* wrapped = holds_wrapped(dst_t, abt_boost::addressof(m_held), abt_boost::addressof(m_held)))
        return wrapped;
    
    type_info src_t = python::type_id<Value>();
    return src_t == dst_t ? abt_boost::addressof(m_held)
        : find_static_type(abt_boost::addressof(m_held), src_t, dst_t);
}
...
# endif
    ...
    {
        python::detail::initialize_wrapper(self, abt_boost::addressof(this->m_held));
    }

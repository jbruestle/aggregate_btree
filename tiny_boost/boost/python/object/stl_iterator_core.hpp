...
struct BOOST_PYTHON_DECL stl_input_iterator_impl
{
    ...
    stl_input_iterator_impl(abt_boost::python::object const &ob);
    void increment();
    bool equal(stl_input_iterator_impl const &that) const;
    abt_boost::python::handle<> const &current() const;
private:
    abt_boost::python::object it_;
    abt_boost::python::handle<> ob_;
};

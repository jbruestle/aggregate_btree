...
// attribute/item access or addition.
  ...
  //
  BOOST_PYTHON_IS_XXX_DEF(proxy, abt_boost::python::api::proxy, 1)
...
# endif
  ...
  struct object_initializer_impl<false, true>
  {
      ...
      get(T const& x, U)
      {
          return python::incref(get_managed_object(x, abt_boost::python::tag));
      }
  };
...
{ 
  ...
  return abt_boost::python::detail::args_proxy(x); 
} 
...
{ 
  ...
  PyObject *result = PyObject_Call(get_managed_object(self, abt_boost::python::tag), 
                                   args.operator object().ptr(), 
                                   0); 
  return object(abt_boost::python::detail::new_reference(result)); 

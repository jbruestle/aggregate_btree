...
class StatsTree:
	def __init__(self, path):
		self.store = btree_c.Store(path, True)
		self.tree = btree.Tree(self.store, "root", StatsAggregate)
		self.counter = 0
	...
	def measures(self, tag = None):
		all_pairs = self.tree.total([tag, None, 0], [tag, '', 0])
		if all_pairs == None:
			all_pairs = {}
	...
	def metrics(self, tag, measures, start, interval, count):
		...
		for i in range(0, count):
			allstats = self.tree.total([tag, start, 0], [tag, start + interval, 0])
			if allstats == None:
				allstats = {}
			row = []
			for m in measures:
